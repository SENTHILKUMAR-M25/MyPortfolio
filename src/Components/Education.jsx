...
const TimelineItem = ({ item, index }) => (
  ...
  >
    ...
    <div className="absolute left-0 top-0 h-full w-[1px] bg-zinc-800 group-last:h-2" />
    
    {/* Node Dot - Animated to look like your logo's sharp style */}
    <div className="absolute left-[-5px] top-2 h-[10px] w-[10px] rounded-sm bg-red-600 rotate-45 border border-black shadow-[0_0_15px_rgba(220,38,38,0.5)] transition-all duration-500 group-hover:scale-150 group-hover:bg-white" />
...
export default function EnhancedCV() {
  ...
  return (
    <div className="bg-[#050505] text-white py-24 px-6 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-red-600/[0.03] rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20 border-b border-zinc-900 pb-12">
          ...
          <div className="flex p-1.5 bg-zinc-900/50 backdrop-blur-xl rounded-xl border border-white/5 relative">
            {["experience", "education"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`relative px-10 py-3 text-xs font-black uppercase tracking-[0.2em] z-10 transition-all duration-500 ${
              ...
              >
                {activeTab === tab && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-r from-red-700 to-red-600 rounded-lg z-[-1] shadow-[0_0_20px_rgba(185,28,28,0.3)]"
